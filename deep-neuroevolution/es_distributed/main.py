...
@click.option('--worker_dir', type=str, default=None)
def workers(algo, master_host, master_port, relay_socket_path, num_workers, worker_dir):
    # Start the relay
    ...
    for _ in range(num_workers):
        if os.fork() == 0:
            algo.run_worker(master_redis_cfg, relay_redis_cfg, noise=noise, worker_dir=worker_dir)
            return
    os.wait()
