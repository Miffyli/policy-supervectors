...
def master():
  ...
  while not es.stop():
    ...
    # like with the NEAT/WANN code
    pop = []
    for solution in solutions:
        wVecCopy = wVec.copy()
        wVecCopy[wKey] = solution
        wVecCopy = wVecCopy.reshape(matDim, matDim)
        pop.append({"wMat": wVecCopy, "aVec": aVec})
    savePop(pop, outPref, gen)
...
def createNet(hyp):
    ...
    aVec = np.full( (np.shape(wMat)[0]), 6) # sigmoid hidden
    aVec[:hyp['ann_nInput']+1] = 1 # Linear inputs
    aVec[-hyp['ann_nOutput']:] = 1 # Linear outputs
